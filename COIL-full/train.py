...
parser = argparse.ArgumentParser(description='COIL-full Training')
        ...
        with tqdm(total=len(data_loader), desc=f"Epoch {epoch + 1}/{epochs}", unit='batch') as pbar:
            for batch in data_loader:
                ...
                if global_step % 500 == 0 and global_step != 0:
                    paddle.save(model.state_dict(), f"{save_path}/COIL-full_checkpoint_{global_step}.pdparams")
...
print("[!] Load model")
model = COIL_full_Model(model_name="COIL-full")
